	...
	else
	{
		...
		{										
			...
			else
			{
				...
				if ($Orders != "")			
					$str = $str . "_Order = '" . replaceSpecialChar($Orders) . "', ";			
				else
					$str = $str . "_Order = null, ";
				
				if ($FooterOrders != "")			
					$str = $str . "_FooterOrder = '" . replaceSpecialChar($FooterOrders) . "', ";			
				else
					$str = $str . "_FooterOrder = null, ";	
			...
			$strSQL = $strSQL . "'Add CMS 1st Level', ";
			if($PageTitle != "")
		...
		else if($e_action == "Edit")
		{
			...
			if($PageTitle != "")
				$str = $str . "_BlockTitle = '" . replaceSpecialChar($PageTitle) . "', ";
			else
				$str = $str . "_BlockTitle = null, ";	
			...
			$strSQL = $strSQL . "'" . replaceSpecialChar($_SERVER['REMOTE_ADDR']) . "', ";
			$strSQL = $strSQL . "'" . date("Y-m-d H:i:s") . "', ";
			$strSQL = $strSQL . "'Edit CMS 1st Level', ";
			if($PageTitle != "")
				$strSQL = $strSQL . "'" . replaceSpecialChar($PageTitle) . "' ";
			else
				$strSQL = $strSQL . "null ";
			...
			include('../dbclose.php');
			echo "<script language='JavaScript'>window.location = '2ndlvlhomecms.php?done=2'</script>";
			exit();
		}
		else if($e_action == "delete")
		{
			...
			$strSQL = $strSQL . "'" . replaceSpecialChar($_SERVER['REMOTE_ADDR']) . "', ";
			$strSQL = $strSQL . "'" . date("Y-m-d H:i:s") . "', ";
			$strSQL = $strSQL . "'Delete CMS 1st Level', ";
			if($PageTitle != "")
				$strSQL = $strSQL . "'" . replaceSpecialChar($PageTitle) . "' ";
			else
				$strSQL = $strSQL . "null ";
		...
		else if($e_action == "cmsupdateorder")
		{
			...
			$strSQL = $strSQL . "'" . replaceSpecialChar($_SERVER['REMOTE_ADDR']) . "', ";
			$strSQL = $strSQL . "'" . date("Y-m-d H:i:s") . "', ";
			$strSQL = $strSQL . "'Update CMS 1st Level Order Display and Footer', ";
			$strSQL = $strSQL . "null ";
			$strSQL = $strSQL . ")";
			mysql_query($strSQL);
		...
		else if($e_action == "AddPreview")
		{
			...
			if($PreviewID=="")
			{
				$str = "INSERT INTO ".$tbname."_cms (_PID, _Level, _PageTitle, _RedirectURL, _Title, _HTMLContent, _Hyperlink, _Order, _FooterOrder, _MetaTitle,_MetaDescription,_MetaKeyWords,_MetaAdditional, _Status, _UserID, _LastUpdatedByID, _IPAddress, _CreatedDate, _LastUpdatedDate) VALUES(";			
				if($PID != "")
					$str = $str . "'" . replaceSpecialChar($PID) . "', ";
				else
					$str = $str . "null, ";
				...
				$str2 = "SELECT MAX(_ID) AS MaxID FROM ".$tbname."_cms";
				$rst2 = mysql_query($str2, $connect) or die(mysql_error());
				if(mysql_num_rows($rst2) > 0)
				{
					$rs2 = mysql_fetch_assoc($rst2);
			...
			else
			{
				...
				$str = $str . "WHERE _ID = '".replaceSpecialChar($PreviewID)."' ";
				mysql_query($str);
				$MaxID = $PreviewID;
			}

			include('../dbclose.php');
			echo "<script language='JavaScript'>window.location = 'add1stlvlcms.php?id=".replaceSpecialChar($MaxID)."'</script>";			
			exit();
		}
		...
		{			
			...
			include('../dbclose.php');
			echo "<script language='JavaScript'>window.location = 'edit1stlvlcms.php?id=".$MaxID."&status=Preview'</script>";			
			exit();
		}
	}
?>
